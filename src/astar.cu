...
int* astar_par(graph &g, int src, int NUM_QUEUES){
	...
	insertKernelAstar <<<1,1>>> (heap, size, src_vertex, 0, g.num_vertices);
	...
	while(!finished){
		...
		extractKernelAstar<<<1, NUM_QUEUES>>>(heap, size, S, W, SWsize, num_connected_d, weight_arr_d,
		...
		deduplicateKernelAstar<<<1, NUM_QUEUES>>>(S, W, SWsize, T, Tsize, visited_d, costs_d, g.num_vertices);
		cudaDeviceSynchronize();

		computeKernelAstar<<<1, NUM_QUEUES>>>(heap, size, T, Tsize, visited_d, costs_d, g.num_vertices);
		cudaDeviceSynchronize();
...
__global__ void extractKernelAstar(int* heap, int* size, int* S, int* W, unsigned int* SWsize, int* num_connected_d, int* weight_arr_d,
	...
	if(size[index] > 0){
		...
		for(int i = 0; i < num_connected_d[current_vertex]; i++){
			...
			S[oldSWsize] = connected_to_d[index_arr_d[current_vertex] + i];
			W[oldSWsize] = weight_arr_d[index_arr_d[current_vertex] + i] + costs_d[current_vertex] + heuristic_GPU(current_vertex);
		}
	}
}
...
__global__ void deduplicateKernelAstar(int* S, int* W, unsigned int* SWsize, int* T, unsigned int* Tsize, bool* visited_d, int* costs_d, int num_vertices){
	...
	for(int i = index; i < SWsize[0]; i+=stride){
		...
		if((visited_d[S[i]] == true) && (costs_d[S[i]] <= W[i])){
			...
			T[oldTsize] = S[i];
			if(costs_d[S[i]] + heuristic_GPU(S[i]) > W[i]){
				atomicExch(&costs_d[S[i]], W[i] - heuristic_GPU(S[i]));
			}
		}
	}
}
