...
int* dijkstra_par(graph &g, int src){
	...
	insert(heap, size[0], src_vertex, 0, g.num_vertices);
	...
	cudaFree(weight_arr_d); cudaFree(connected_to_d);
	delete [] visited;
...
__global__ void extractKernelDijkstra(int* heap, int* size, int* S, int* W, unsigned int& SWsize, int* num_connected_d, int* weight_arr_d,
	...
	if(size[index] > 0){
		...
		for(int i = 0; i < num_connected_d[current_vertex]; i++){
			oldSWsize = atomicInc(&SWsize, 1);
			S[oldSWsize] = connected_to_d[index_arr_d[current_vertex] + i];
			W[oldSWsize] = weight_arr_d[S[oldSWsize]] + costs_d[current_vertex];
		}
	}
}
...
__global__ void deduplicateKernelDijkstra(int* S, int* W, unsigned int& SWsize, int* T, unsigned int& Tsize, bool* visited_d, int* costs_d, int num_vertices){
	...
	for(int i = index; i < SWsize; i+=stride){
		if(visited_d[S[i]] && costs_d[S[i]] < W[i]){
			continue;
		}
		else{
			oldTsize = atomicInc(&Tsize, 1);
